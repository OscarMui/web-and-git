...
\usepackage{float} % to fix the position of tables and images, so that it won't go to another place
...
% keep lstlisting tabs when copying code from pdf
...
\lstdefinelanguage{pug}{
  ...
  keywordstyle=\bfseries,
  ndkeywords={src, href, target, width, height, myCarousel},
  ndkeywordstyle=\bfseries,
  identifierstyle=\color{black},
  sensitive=false,
  comment=[l]{//-},
...
\lstdefinelanguage{bash}{
  keywords={npm, git, add, commit, push, pull, fetch, clone, init, checkout, merge, cd, ls, rm, cp, mv, pwd, mkdir, touch},
  keywordstyle=\bfseries,
  ndkeywords={},
  ndkeywordstyle=\bfseries,
  identifierstyle=\color{black},
...
\degree{Computer Science } %\\[1ex] (in Python)     %the degree
